...
/*
  IdbFactory.supported => WebDatabase.withStorage
...
Future<QueryExecutor> $createQueryExecutor({
  ...
  */
  if (dropDatabase) {
    html.window.indexedDB!.deleteDatabase(Config.secureDatabaseName);
  }
  final result = await wasm.WasmDatabase.open(
    databaseName:
        memoryDatabase ? ':memory:' : path ?? Config.secureDatabaseName,
